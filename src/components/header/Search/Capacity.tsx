...
export const Capacity = ({ form }: SearchFormProps) => {
  ...
  const popoverTitle = () => {
    ...
    if (!seats && !rooms) {
      return t('search:capacity.title');
    }
    if (!seats) {
      return `${rooms} ${t('search:capacity.room', {
        count: rooms,
      })}`;
    } else if (!rooms) {
      return `${seats} ${t('search:capacity.seat', {
        count: seats,
      })}`;
    } else {
      return `${seats} ${t('search:capacity.seat', {
        count: seats,
      })}, ${rooms} ${t('search:capacity.room', {
        count: rooms,
      })}`;
    }
  };
